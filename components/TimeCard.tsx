...
import { Clock, CheckCircle2, Timer, Info } from 'lucide-react';
...
const TimeCard: React.FC<TimeCardProps> = ({ status, onClick, isSelected, isNext, variant = 'grid', t, theme }) => {
  ...
  if (isList) {
    return (
      <div 
        ...
        className={`relative overflow-hidden cursor-pointer transition-all duration-300 p-3 rounded-lg border flex items-center justify-between gap-4 backdrop-blur-sm hover:scale-[1.02] hover:shadow-lg ${
          status.isIn2026 
            ? (isDark ? 'bg-emerald-950/40 border-emerald-500/30 hover:bg-emerald-900/50' : 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100')
            : (isNext 
                ? (isDark ? 'bg-blue-900/20 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-blue-50 border-blue-300 shadow-blue-100')
                : (isDark ? 'bg-slate-900/40 border-slate-700/50 hover:border-blue-500/50 hover:bg-slate-800/50' : 'bg-white border-slate-200 hover:border-blue-300 hover:bg-slate-50 shadow-sm')
              )
        } ${isSelected ? 'ring-2 ring-blue-500' : ''} group`}
      >
        {/* Left: Identity */}
        <div className="flex items-center gap-3 min-w-0">
          <div className={`flex-shrink-0 w-8 h-6 rounded flex items-center justify-center border overflow-hidden shadow-sm ${isDark ? 'bg-slate-800 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
            <img 
          ...
          <div className="min-w-0 flex flex-col">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`md:hidden p-0.5 rounded-full transition-colors ${isDark ? 'text-slate-500 hover:text-blue-400' : 'text-slate-400 hover:text-blue-500'}`}
          ...
          <button
            ...
            className={`hidden md:flex p-1.5 rounded-full transition-colors ${isDark ? 'text-slate-500 hover:bg-white/10 hover:text-blue-400' : 'text-slate-400 hover:bg-slate-100 hover:text-blue-500'}`}
  ...
  return (
    <div 
      onClick={() => onClick(status.country)}
      className={`relative overflow-hidden cursor-pointer transition-all duration-300 p-4 rounded-xl border backdrop-blur-sm hover:scale-[1.02] hover:shadow-xl ${
        status.isIn2026 
          ? (isDark ? 'bg-emerald-950/40 border-emerald-500/50 hover:bg-emerald-900/50' : 'bg-emerald-50/80 border-emerald-200 hover:bg-emerald-100/50') 
          : (isNext
              ? (isDark ? 'bg-blue-900/20 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.15)] hover:bg-blue-900/30' : 'bg-blue-50 border-blue-300 shadow-blue-100 hover:bg-blue-100/50')
              : (isDark ? 'bg-slate-900/40 border-slate-700/50 hover:border-blue-500/50 hover:bg-slate-800/50' : 'bg-white/70 border-slate-200 hover:border-blue-300 hover:bg-white shadow-sm')
            )
      } ${isSelected ? 'ring-2 ring-blue-500' : ''} group`}
    >
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="flex items-center gap-2">
           <button
             ...
             className={`p-1 rounded-full transition-colors ${isDark ? 'text-slate-500 hover:bg-white/10 hover:text-blue-400' : 'text-slate-400 hover:bg-slate-100 hover:text-blue-500'}`}
             title={t.learnTraditions}
           >
             <Info size={16} />
           </button>
           ...
           {status.isIn2026 ? (
             <CheckCircle2 className="text-emerald-500 w-5 h-5 flex-shrink-0 animate-pulse" />
           ) : (
             <Timer className={`w-5 h-5 flex-shrink-0 ${isNext ? 'text-blue-400 animate-pulse' : 'text-blue-500'}`} />
           )}
        </div>
      </div>

      {isNext && (
          <div className={`absolute top-0 right-0 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider rounded-bl-lg ${isDark ? 'bg-blue-500 text-white' : 'bg-blue-500 text-white'}`}>
             Next Arrival
          </div>
      )}

      {/* Body: Two-column grid for Time and Countdown */}
      <div className={`grid grid-cols-2 gap-2 pt-3 border-t ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
